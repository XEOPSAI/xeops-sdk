...
type BlessedLike = {
  screen: (config: Record<string, unknown>) => any;
  log: (config: Record<string, unknown>) => any;
  textbox: (config: Record<string, unknown>) => any;
};
...
export function parseInteractiveCommand(raw: string): InteractiveCommand {
  ...
  if (normalized.startsWith('focus ')) return { type: 'focus', value: normalized.slice(6).trim() || DEFAULT_FOCUS };
  if (normalized.startsWith('skip ')) return { type: 'skip', value: normalized.slice(5).trim() };
...
function renderFindings(result: ScanResult, state: InteractiveState, writeLine: (line: string) => void): void {
  ...
  const rendered = visibleFindings.slice(0, 20).map(formatFindingLine);
...
async function runWithBlessedUi(
  ...
  const log = blessed.log({ top: 0, left: 0, width: '100%', height: '90%', border: 'line', scrollable: true });
  const inputBox = blessed.textbox({ bottom: 0, left: 0, width: '100%', height: '10%', border: 'line', inputOnFocus: true });
  screen.append(log);
...
function readBlessedCommand(inputBox: any): Promise<InteractiveCommand> {
